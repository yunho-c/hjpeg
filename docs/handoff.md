...
- JSON evidence records the bitstream/XSA/report SHA-256 hex hashes, PPM/RGB
  ...
  nonempty TX/RX device paths, their resolved identities, and distinct raw and
  resolved endpoints. Capture configuration evidence must
...
evidence path lists, parsed HJPEG base addresses, HJPEG base-address count and
consistency flag, plus the recomputed summary, evidence/check counts, missing
