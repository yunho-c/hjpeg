...
order, records SOF0 component sampling factors, records JFIF APP0 signature
...
- `validate-jpeg` confirms the expected dimensions, 8-bit SOF0 sample
  ...
  coverage, SOF0 component sampling factors, chroma mode, DRI restart interval,
  restart markers, JFIF APP0 signature presence, and valid DQT/DHT table
...
- JSON evidence records the bitstream/XSA/report hashes, PPM/RGB input hashes,
  ...
  component coverage, component sampling factors, marker counts, JFIF APP0
  signature count, restart interval evidence, DQT/DHT table IDs, SOS table
  selectors, chroma mode, JFIF evidence, and decoder command.
