...
- The JSON evidence records artifact/report paths, byte lengths, SHA-256 hex
  ...
  aggregate pass/fail counts, diagnostic failure count, passed/failed path
  lists, and pass/fail state. Complete Vivado flow evidence has
...
- `run-stream-devices --json` evidence includes `hardware_run_summary` with
  ...
  counts, missing evidence group names, failing check names, and
...
- Saved run JSON can be checked later with `check-run-evidence`, which fails if
  ...
  passed/failed path lists plus the recomputed summary, evidence/check counts,
