...
gates, failed recorded checks, or incomplete hardware evidence. Saved run and
Vivado evidence files must be strict JSON; non-standard constants such as
`NaN` and `Infinity` are rejected as malformed evidence. With
