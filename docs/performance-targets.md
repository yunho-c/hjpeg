...
This document turns the project's general throughput goal into measurable
...
- fixture: deterministic `gradient-checker` RGB generated by
  `hjpeg_host.py make-test-ppm`, quality 85, no restart interval, JFIF enabled;
...
Because JPEG work and output byte count depend on content and quality, this is a
defined benchmark rather than a content-independent worst-case guarantee. The
quality-90 `seeded-random` fixture is retained as a reproducible stress case.
...
| --- | ---: | ---: |
| Four-lane 8x8 DCT block latency | 38 | 38 |
| Consecutive DCT block initiation | 16 | 16 |
| Four-lane 64-coefficient quantization | 23 | 23 |
| Complete DCT/quantize/zig-zag block latency | 61 | 61 |
| Four-block transform initiation intervals | 16/16/16 | 17 maximum |
| First 4:4:4 MCU after stripe ownership handoff | 104 | 104 |
| First 4:2:0 MCU after band ownership handoff | 272 | 272 |
| Complete 16x16 4:4:4 test frame | 2,036 | 2,300 |
...
change. Pair formation and exact two-product partial sums are registered before
the final add, row and column engines overlap through three banked transpose
buffers, and two result banks absorb output
...
previous production path, DCT latency fell from 129 to 38 cycles, quantization
from 66 to 23, and complete transform latency from 196 to 61. First-MCU
...
The current timing-register depth makes the isolated boundaries 104 and 272
cycles and the complete frame fixture 2,036 cycles.
...
The exact instrumented RTL passes the complete routed Vivado evidence gate at
100 MHz with setup WNS `+0.097 ns` and hold WHS `+0.010 ns`. The
26-bit AXI DMA length configuration allows one packed 1920x1080 frame per MM2S
transaction. The routed design uses 35,885 CLB LUTs (30.64%), 55,074 registers
(23.51%), 76 BRAM tiles (52.78%), 127 DSPs (10.18%), and 8,224 physical CLBs
(56.17%).

A physical KV260 revB run measured complete frame latency from the first
accepted input beat through accepted JPEG TLAST with a 64-bit PL counter. Both
DMA channels ended IOC/idle, encoder status returned to zero, and every captured
stream passed strict marker/table validation and FFmpeg decoding:

| Fixture | Mode | Cycles | Time at 100 MHz | FPS | JPEG bytes | Result |
| --- | --- | ---: | ---: | ---: | ---: | --- |
| gradient-checker q85 | 4:2:0 | 2,210,885 | 22.108850 ms | 45.230756 | 151,020 | target pass |
| gradient-checker q85 | 4:4:4 | 3,224,557 | 32.245570 ms | 31.012012 | 168,562 | target pass |
| seeded-random q90 | 4:2:0 | 2,211,207 | 22.112070 ms | 45.224169 | 1,702,123 | decoder/target pass |
| seeded-random q90 | 4:4:4 | 3,734,574 | 37.345740 ms | 26.776816 | 3,021,074 | below 30 fps |

The stress result is expected to be more output/entropy sensitive: its 4:4:4
scan contains 2,973,135 bytes plus 47,314 stuffed bytes. A trial 32-entry run
FIFO improved the 256x64 simulation by only 139 cycles (0.47%) because the run
producer was already never backpressured, so it was not retained. Improving
this boundary requires wider/multi-token entropy handling, a wider/faster output
path, or a narrower quality/content guarantee rather than more raster depth.
...
1. production Linux DMA/driver integration that preserves the measured PL-cycle
   evidence without stopping the application processor;
2. wider or multi-token entropy/output architecture only if the project adopts
   a high-entropy 4:4:4 worst-case target; and
3. broader large-content traces only for a concrete unresolved hypothesis.
