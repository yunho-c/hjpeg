...
class HjpegHostTest(unittest.TestCase):
    ...
    def test_hardware_summary_requires_strict_dimension_evidence(self) -> None:
        record = {
            "width": 1,
            "height": 1,
            "byte_length": 16,
            ...
            "marker_sequence": ["SOI", "SOS", "EOI"],
            **baseline_marker_count_record(),
            "encoder_config": {"width": True, "height": True},
            "validation_expectations": {"width": True, "height": True},
            "input_ppm": {"width": True, "height": True},
        }
    ...
    def test_vivado_evidence_file_record_rejects_extra_checked_count_categories(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "vivado.json"
            vivado_record = vivado_evidence_record(0)
            vivado_record["checked_counts"]["unexpected"] = 1
            path.write_text(json.dumps(vivado_record))
