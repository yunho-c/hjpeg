...
class VivadoScriptsSpec extends AnyFreeSpec with Matchers {
  ...
  "Vivado scripts should target the generated KV260 AXI-Lite top" in {
    ...
    blockDesign must include("Expected at most 4 arguments: ip_repo_dir project_dir mm2s_data_width pl_clock_mhz")
    ...
    blockDesign must include("CONFIG.c_include_mm2s_sf {0}")
    blockDesign must include("CONFIG.c_mm2s_burst_size {256}")
    ...
    blockDesign must include("set pl_clock_mhz 100")
    blockDesign must include("set pl_clock_mhz [lindex $argv 3]")
    blockDesign must include("PS pl_clk0 frequency must be a positive integer in MHz")
    blockDesign must include("CONFIG.PSU__CRL_APB__PL0_REF_CTRL__FREQMHZ $pl_clock_mhz")
    ...
    xsdbDma must include("set width [parse_unsigned width [lindex $argv 3] 3840]")
    xsdbDma must include("set height [parse_unsigned height [lindex $argv 4] 2160]")
    xsdbDma must include("set output_addr [expr {$argc >= 11 ? [parse_unsigned output_addr [lindex $argv 10] 0xFFFFFFFF] : 0x64000000}]")
    xsdbDma must include("set output_capacity [expr {$argc >= 12 ? [parse_unsigned output_capacity [lindex $argv 11] 0x03FFFFFF] : 0x03FFFFFF}]")
    xsdbDma must include("HJPEG_XSDB_PREFLIGHT_ONLY")
    xsdbDma must include("PREFLIGHT_OK")
    xsdbDma must include("targets -set -filter {name == \"Cortex-A53 #0\"}")
    xsdbDma must include("catch {stop}")
    xsdbDma must include("targets -set -filter {name == \"APU\"}")
    xsdbDma.indexOf("targets -set -filter {name == \"APU\"}") must be <
      xsdbDma.indexOf("dow -data $input_rgb $input_addr")
    ...
    xsdbDma must include("clock_hz=%d milliseconds=%.6f fps=%.6f max_frame_cycles=%d")
    xsdbDma must include("if {!$frame_target_met}")
    xsdbDma must include("frame latency $frame_cycles cycles exceeds target $max_frame_cycles cycles")
