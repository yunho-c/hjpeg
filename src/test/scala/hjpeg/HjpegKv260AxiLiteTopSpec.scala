...
class HjpegKv260AxiLiteTopSpec extends AnyFreeSpec with Matchers with ChiselSim {
  ...
  "HjpegKv260AxiLiteTop should recover after an early TLAST frame is cleared" in {
    simulate(new HjpegKv260AxiLiteTop(HjpegConfig(maxFrameWidth = 32, maxFrameHeight = 32))) { dut =>
      dut.reset.poke(true.B)
      dut.clock.step()
      dut.reset.poke(false.B)
  ...
  "HjpegKv260AxiLiteTop should report unsupported frame dimensions through AXI-Lite status" in {
    simulate(new HjpegKv260AxiLiteTop(HjpegConfig(maxFrameWidth = 32, maxFrameHeight = 32))) { dut =>
      dut.reset.poke(true.B)
      dut.clock.step()
      dut.reset.poke(false.B)
  ...
  "HjpegKv260AxiLiteTop should reject missing lower RGB keep bits" in {
    simulate(new HjpegKv260AxiLiteTop(HjpegConfig(maxFrameWidth = 32, maxFrameHeight = 32))) { dut =>
      dut.reset.poke(true.B)
      dut.clock.step()
      dut.reset.poke(false.B)
